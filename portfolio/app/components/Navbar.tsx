  ...
  const items = [
    {
      ...
      </svg>), tooltip: "Home", link: "/"
    },
    {
      svg: (<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="size-6">
        <path fillRule="evenodd" d="M7.5 6a4.5 4.5 0 1 1 9 0 4.5 4.5 0 0 1-9 0ZM3.751 20.105a8.25 8.25 0 0 1 16.498 0 .75.75 0 0 1-.437.695A18.683 18.683 0 0 1 12 22.5c-2.786 0-5.433-.608-7.812-1.7a.75.75 0 0 1-.437-.695Z" clipRule="evenodd" />
      </svg>), tooltip: "About", link: "/about"
    },
    {
      svg: (<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="size-6">
    ...
    },
    {
      ...
      </svg>), tooltip: "Contact", link: "/contact"
    },
  ]
      ...
      <div className="flex gap-3">
        {items.map((item, index) => (
          <div key={index} className="relative group">
            <Link to={item.link} className="flex justify-center items-center bg-[#9df9ef] hover:bg-opacity-50 cursor-pointer h-12 w-12 rounded-md text-white">
              {item.svg}
            </Link>
            <div className="absolute top-full left-1/2 transform -translate-x-1/2 mt-2 bg-gray-800 text-white text-sm rounded px-2 py-1 opacity-0 transition-opacity duration-200 group-hover:opacity-100">
              {item.tooltip}
            </div>
          </div>
        ))}
